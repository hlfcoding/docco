(function() {
  var destination, docco_client_scripts, docco_styles, docco_template, ensure_directory, exec, ext, fs, generate_documentation, generate_html, get_directory_files, get_language, highlight, highlight_end, highlight_start, l, languages, parse, path, showdown, sources, spawn, template, _ref;
  ...
  destination = function(filepath) {
    return 'docs/' + filepath.replace(/\//g, '.') + '.html';
  };
  ...
  get_directory_files = function(dir, callback) {
    return fs.readdirSync(dir).forEach(function(file) {
      ...
      if (fstat.isDirectory()) {
        return get_directory_files(fpath, callback);
      }
    });
  };
  ...
  docco_client_scripts = (function() {
    var includes;
    includes = [];
    get_directory_files(__dirname + '/../vendor/client', function(fullPath) {
      return includes.push(fs.readFileSync(fullPath).toString());
    });
